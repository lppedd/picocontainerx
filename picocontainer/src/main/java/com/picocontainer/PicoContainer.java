...
public interface PicoContainer {
   ...
   * If a component cannot be found in this container, the parent container (if one exists) will be searched.
   *
   ...
   * key.
   */
  Object getComponent(final Object keyOrType);

  Object getComponentInto(final Object keyOrType, final Type into);
  ...
  <T> T getComponent(final Generic<T> componentType);
   ...
   * @return the typed resulting object instance or null if the object does not exist.
   */
  <T> T getComponentInto(final Class<T> componentType, final Type into);

  <T> T getComponentInto(final Generic<T> componentType, final Type into);
   ...
   * @return the typed resulting object instance or null if the object does not exist.
   */
  <T> T getComponent(final Class<T> componentType, final Class<? extends Annotation> binding, Type into);

  <T> T getComponent(final Class<T> componentType, final Class<? extends Annotation> binding);
   ...
   *
   * @return a {@code PicoContainer} instance, or {@code null} if this container does not have a parent.
   */
   ...
   */
  ComponentAdapter<?> getComponentAdapter(final Object key);

  /**
   * Find a component adapter associated with the specified type and binding name.
   * If a component adapter cannot be found in this container, the parent container (if one exists) will be searched.
   *
   * @param componentType the type of the component.
   * @param nameBinding the name binding to use. May be {@link NameBinding#NULL}
   ...
   * registered for the specified key.
   */
  <T> ComponentAdapter<T> getComponentAdapter(final Class<T> componentType, final NameBinding nameBinding);
   ...
   * registered for the specified key.
   */
  <T> ComponentAdapter<T> getComponentAdapter(final Generic<T> componentType, final NameBinding nameBinding);
   ...
   */
  <T> ComponentAdapter<T> getComponentAdapter(
   ...
   * The component adapters from the parent container are not returned.
   *
   * @return a collection containing all the {@link ComponentAdapter}s inside this container.
   * The collection will not be modifiable.
   *
   ...
   * The {@code addComponent} adapters from the parent container are not returned.
   *
   ...
   */
  <T> List<ComponentAdapter<T>> getComponentAdapters(final Class<T> componentType);

  <T> List<ComponentAdapter<T>> getComponentAdapters(final Generic<T> componentType);
   ...
   * the specified type. Changes to this collection will not be reflected in the container itself.
   */
  <T> List<ComponentAdapter<T>> getComponentAdapters(
      final Class<T> componentType,
      final Class<? extends Annotation> binding
  );
   ...
   * The list is ordered by instantiation order, starting with the components instantiated first at the beginning.
   *
   ...
   */
  <T> List<T> getComponents(final Class<T> componentType);
