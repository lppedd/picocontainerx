...
/**
 * An {@link InjectionType} for constructor injection.
...
public class ConstructorInjection extends AbstractInjectionType {
  ...
  public <T> ComponentAdapter<T> createComponentAdapter(
    ...
    final boolean useNames = AbstractBehavior.arePropertiesPresent(properties, Characteristics.USE_NAMES, true);
    final ConstructorInjector<T> injector = newConstructorInjector(monitor, key, impl, useNames, constructorParams);
  ...
  protected <T> ConstructorInjector<T> newConstructorInjector(
      final ComponentMonitor monitor,
      final Object key,
      final Class<T> impl,
      final boolean useNames,
      final ConstructorParameters parameters) {
    return new ConstructorInjector<>(monitor, useNames, rememberChosenConstructor, key, impl, parameters);
  }
  ...
  public static class ConstructorInjector<T> extends MultiArgMemberInjector<T> {
    ...
    public ConstructorInjector(
        final Object key,
        final Class<T> impl,
        @Nullable final ConstructorParameters parameters) {
     ...
     *
     * @throws NotConcreteRegistrationException if the implementation
    ...
    protected CtorAndAdapters<T> getGreediestSatisfiableConstructor(final PicoContainer container) throws PicoCompositionException {
      ...
      final Map<ResolverKey, Resolver> resolvers = new HashMap<>();
      ...
      for (final Constructor<T> sortedMatchingConstructor : sortedMatchingConstructors) {
        try {
          ...
          final Type[] parameterTypes = sortedMatchingConstructor.getGenericParameterTypes();
          fixGenericParameterTypes(sortedMatchingConstructor, parameterTypes);
          final Annotation[] bindings = getBindings(sortedMatchingConstructor.getParameterAnnotations());
          ...
          final List<Parameter> parametersUsed = new ArrayList<>(Arrays.asList(currentParameters));

          // remember: all constructors with less arguments than the given parameters are filtered out already
          for (int j = 0; j < currentParameters.length; j++) {
            lastParameterTested = j;
            // check whether this constructor is satisfiable
            final Type expectedType = box(parameterTypes[j]);
            final NameBinding expectedNameBinding = new ParameterNameBinding(getParanamer(), sortedMatchingConstructor, j);
            final Parameter parameterToUse = getParameterToUse(sortedMatchingConstructor, j, currentParameters[j]);
            parametersUsed.set(j, parameterToUse);
            final ResolverKey resolverKey = new ResolverKey(expectedType, useNames() ? expectedNameBinding.getName() : null, useNames(), bindings[j], parameterToUse);
            Resolver resolver = resolvers.get(resolverKey);
            if (resolver == null) {
              final Parameter currentParameter = parameterToUse;
              final Annotation annotation = bindings[j];
              final boolean b = useNames();
      ...
      if (!conflicts.isEmpty()) {
        ...
        throw new UnsatisfiableDependenciesException(getComponentImplementation().getName()
        ...
        final Set<Constructor> nonMatching = new HashSet<>();
        nonMatching.addAll(Arrays.asList(getConstructors()));
        throw new PicoCompositionException("Either the specified parameters do not match any of the following constructors: " + nonMatching + "; OR the constructors were not accessible for '" + getComponentImplementation().getName() + "'");
      }
      return new CtorAndAdapters<>(greediestConstructor, greediestConstructorsParameters, greediestConstructorsParametersComponentAdapters);
    }

    /**
     * Allows for subclasses to override the {@link Parameter} for a given constructor argument.
     *
     ...
     * @return
     */
    protected Parameter getParameterToUse(
        final Constructor<?> constructorToExamine,
        final int constructorParameterIndex,
    ...
    private static final class ResolverKey {
      ...
      public boolean equals(final Object o) {
        ...
        final ResolverKey that = (ResolverKey) o;
    ...
    protected class CtorAndAdapters<TYPE> {
      ...
      public CtorAndAdapters(
          final Constructor<TYPE> ctor,
          final Parameter[] parameters,
          final ComponentAdapter[] injecteeAdapters) {
        this.ctor = ctor;
      ...
      public Object[] getParameterArguments(final PicoContainer container, final Type into) {
        ...
        final Object[] result = new Object[constructorParameters.length];
        final Annotation[] bindings = getBindings(ctor.getParameterAnnotations());
        ...
        if (instantiationGuard == null) {
          ...
          instantiationGuard = new ThreadLocalCyclicDependencyGuard<T>() {
            ...
            public T run(final Object instance) {
              final CtorAndAdapters<T> ctorAndAdapters = getGreediestSatisfiableConstructor(guardedContainer, getComponentImplementation());
              final ComponentMonitor monitor = currentMonitor();
              Constructor<T> ctor = ctorAndAdapters.getConstructor();
              try {
                final Object[] ctorParameters = ctorAndAdapters.getParameterArguments(guardedContainer, into);
                ...
                final T inst = newInstance(ctor, ctorParameters);
    ...
    private List<Constructor<T>> getSortedMatchingConstructors() {
      final List<Constructor<T>> matchingConstructors = new ArrayList<>();
      final Constructor<T>[] allConstructors = getConstructors();
      ...
      for (final Constructor<T> constructor : allConstructors) {
    ...
    private Constructor<T>[] getConstructors() {
      return AccessController.doPrivileged((PrivilegedAction<Constructor<T>[]>) () ->
          (Constructor<T>[]) getComponentImplementation().getDeclaredConstructors()
      );
    }
